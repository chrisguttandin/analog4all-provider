...
module.exports = (grunt) => {
    return {
        'assets': {
            files: {
                './': [
                    'build/analog4all-provider/browser/index.html',
                    'build/analog4all-provider/browser/*.css',
                    'build/analog4all-provider/browser/*.js'
                ]
            },
            options: {
                patterns: [
        ...
        'csp-production': {
            files: {
                'build/analog4all-provider/browser/index.html': ['build/analog4all-provider/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/analog4all-provider/browser/index.html', 'utf8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': ['build/analog4all-provider/browser/ngsw.json']
            },
            options: {
                patterns: [
                    {
                    ...
                    },
                    {
                        ...
                        replacement: (_, filename) => {
                            return `"/analog4all-provider/browser/${filename}.html": "${computeHashOfFile(
                                `build/analog4all-provider/browser/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
                ...
                patterns: [
                    {
                        ...
                        replacement: (_, filename, moduleAttribute) => {
                            if (moduleAttribute === undefined) {
                                return `<script>${fs.readFileSync(`build/analog4all-provider/browser/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                            }

                            return `<script${moduleAttribute}>${fs.readFileSync(`build/analog4all-provider/browser/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                        }
                    }
                ]
            }
